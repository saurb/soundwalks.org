        ...
        comparisons.each_with_index do |sounds_to_compare, i|
          ...
          sounds_to_compare.each do |j|
            ...
            if !cost.nan? && cost < Infinity && cost >= 0
              ...
              puts "\t\tInvalid cost #{i}, #{j} (sound #{sounds[i].id}/node #{sounds[i].mds_node.id}, sound #{sounds[j].id}/node #{sounds[j].mds_node.id}): (self[i]: #{self_comparisons[i]}, self[j]: #{self_comparisons[j]})"
            end
          end
        end
      end
