   ...
   costs.each do |source, dummy|
     ...
     Link.transaction do
       shortest.each do |id, distance|
         Link.update(link_ids[source][id], :distance => distance) if link_ids[source][id]
       end
     end
     ...
     Link.transaction do 
       shortest.each do |id, distance|
         Link.create(:first_id => source, :second_id => id, :cost => nil, :distance => distance) if !link_ids[source][id]
       end
     end
     
     puts "#{index + 1} / #{costs.size}"
     index += 1
   end
