...
package com.clevr.matrixalgebra {
	...
	public class RealMatrix {
		...
		public function getElement (i:int, j:int):Number {
			return A[i][j];
		}
		...
		*/
		...
		//the name of this function modified by A. Fink (11/6/09) 
		public function setElement (i:int, j:int, s:Number):void {
			A[i][j] = s;
		}
		...
		*/
		...
		/** Solve X*A = B	least squares solution for rank deficient A
		 	@param B 	right hand side
		 	@return		least squares solution
		 */
		
		public function solve_lowrank (B:RealMatrix):RealMatrix {
			 	
			 	var svd:SingularValueDecomposition = new SingularValueDecomposition(this);
			 	var R:int = rank();   			
      			var U_orig:RealMatrix = svd.getU();
      			var U:RealMatrix = new RealMatrix(m,R,0);
      			var S:Array = svd.getSingularValues();
      			var V_orig:RealMatrix = svd.getV();
      			var V:RealMatrix = new RealMatrix(n,R,0);
      			var y:RealMatrix = new RealMatrix(1,1,0);
     		
      			for (var j1:int = 0; j1 < R; j1++)
      			{
      				var sqrtS = Math.sqrt(S[j1]);
      				for(var i1:int = 0; i1 < m; i1++)
      				{
      					U.setElement(i1,j1,(sqrtS*U_orig.getElement(i1,j1)));
      				}
      				for(var i1:int = 0; i1 < n; i1++)
      				{
      					V.setElement(i1,j1,(sqrtS*V_orig.getElement(i1,j1)));
      				}
      			}
